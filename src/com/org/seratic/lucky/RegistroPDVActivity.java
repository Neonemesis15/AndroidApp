...
public class RegistroPDVActivity extends Activity implements IComunicacionListener {
	...
	private static final int MIN_DNI = 8;
	private static final int MIN_RUC = 11;
	private static final int MIN_DIRECCION = 3;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.ly_registro_pdv);

		txtnombre = (EditText) findViewById(R.id.etNombre);
		txtnombre.setFilters(new InputFilter[] { filter });
		// txtnombre.addTextChangedListener(new CustomTextWatcher(txtnombre));
		...
		txtDireccion.setFilters(new InputFilter[] { filter });
		...
		locationHandler = new MarcacionLocationHandler(db, RegistroPDVActivity.this);
	...
	private void fillPoblacionSpinner() {
		Spinner spPoblacion = (Spinner) findViewById(R.id.spTipoPoblacion);
	...
	private void fillDepartamentosSpinner() {
		...
		List<Entity> departamentos = departamentoController.getAll(COD_PAIS_PERU);
		Log.d(TAG, "Departamentos cargados  : " + departamentos.size());
		ArrayAdapter<Entity> departamentoAdapter = new ArrayAdapter<Entity>(this, android.R.layout.simple_spinner_item, departamentos);
	...
	private void fillProvinciaSpinner() {

		spProvincia = (Spinner) findViewById(R.id.spProvincia);
		E_TblMstDepartamento departamento = (E_TblMstDepartamento) spDepartamento.getSelectedItem();
		Log.d(TAG, "Departamento seleccionado " + departamento.getDepartamento() + " cod " + departamento.getCod_departamento());
		List<Entity> provincias = provinciaController.getAll(COD_PAIS_PERU, departamento.getCod_departamento());
		Log.d(TAG, "Provincias cargadas " + provincias.size());
		ArrayAdapter<Entity> provinciaAdapter = new ArrayAdapter<Entity>(this, android.R.layout.simple_spinner_item, provincias);
	...
	private void fillDistritoSpinner() {
		spDistrito = (Spinner) findViewById(R.id.spDistrito);
		E_TblMstDepartamento departamento = (E_TblMstDepartamento) spDepartamento.getSelectedItem();
		E_TblMstProvincia provincia = (E_TblMstProvincia) spProvincia.getSelectedItem();

		Log.d(TAG, "Departamento seleccionado " + departamento.getDepartamento() + " cod " + departamento.getCod_departamento());
		Log.d(TAG, "Provincia seleccionada " + provincia.getProvincia() + " cod " + provincia.getCod_provincia());

		List<Entity> distritos = distritoController.getAll(COD_PAIS_PERU, departamento.getCod_departamento(), provincia.getCod_provincia());

		ArrayAdapter<Entity> distritoAdapter = new ArrayAdapter<Entity>(this, android.R.layout.simple_spinner_item, distritos);
	...
	private boolean guardarPDV() {
		...
		codigosITT = DatosManager.getInstancia().getCodigosITT();
		TblPuntoGPS puntoGPS = GPSManager.getManager().getPuntoGPS(db, RegistroPDVActivity.this, true);
	...
	private E_TblMovRegPDV fillE_TblMovRegPDV() {
		...
		if (spTipoDocumento.getSelectedItemPosition() == 0) {
		...
		vo.setDireccion(txtDireccion.getText().toString());
		...
		vo.setCod_departamento(((E_TblMstDepartamento) spDepartamento.getSelectedItem()).getCod_departamento());
		vo.setCod_provincia(((E_TblMstProvincia) spProvincia.getSelectedItem()).getCod_provincia());
		vo.setCod_distrito(((E_TblMstDistrito) spDistrito.getSelectedItem()).getCod_distrito());

		Spinner spTipo = (Spinner) findViewById(R.id.spTipoPoblacion);

		// Modificar el metodo getall, hay un error en el cursor y en codigo
		// pone nombre de distribuidora
		vo.setCod_poblacion(((E_TblMstPoblacion) spTipo.getSelectedItem()).getCod_poblacion());
	...
	private void callNoImplementacion() {
		finish();
		Intent intent = new Intent(RegistroPDVActivity.this, MotivoNoVisitaBodega.class);
	...
	private void clearForm() {
		...
		EditText etRUC = (EditText) findViewById(R.id.etRUC);
				...
				if (txtApellido.getText().toString().length() >= MIN_APELLIDO) {
					if (txtRazon.getText().toString().length() >= MIN_RAZON) {
						...
						if (spTipoDocumento.getSelectedItemPosition() == 0) {
							...
							if (etDNI.getText().toString().length() >= MIN_DNI) {
								if (txtDireccion.getText().toString().length() >= MIN_DIRECCION) {
									if (txtReferencia.getText().toString().length() >= MIN_REFERENCIA) {
										if (etTelefono.getText().toString().length() >= MIN_TELEFONO) {
											return true;
										} else {
											etTelefono.requestFocus();
											etTelefono.startAnimation(shake);
											toastError = Toast.makeText(getApplicationContext(), getString(R.string.errorMinLenght) + " " + MIN_TELEFONO, Toast.LENGTH_SHORT);
											toastError.show();
											return false;
										}
									} else {
									...
									txtDireccion.startAnimation(shake);
									toastError = Toast.makeText(getApplicationContext(), getString(R.string.errorMinLenght) + " " + MIN_DIRECCION, Toast.LENGTH_SHORT);
									toastError.show();
									return false;
								}
							} else {
								...
								toastError = Toast.makeText(getApplicationContext(), getString(R.string.errorMinDoc) + " " + MIN_DNI + " digitos.", Toast.LENGTH_SHORT);
								toastError.show();
								return false;
							}
						} else if (spTipoDocumento.getSelectedItemPosition() == 1) {
							// if (etRUC.getText().toString().length() == 0 ||
							// etRUC.getText().toString().length() >= MIN_RUC)
							if (etRUC.getText().toString().length() >= MIN_RUC) {
								if (txtDireccion.getText().toString().length() >= MIN_DIRECCION) {
									if (txtReferencia.getText().toString().length() >= MIN_REFERENCIA) {
										if (etTelefono.getText().toString().length() >= MIN_TELEFONO) {
											return true;
								...
								toastError = Toast.makeText(getApplicationContext(), getString(R.string.errorMinDoc) + " " + MIN_RUC + " digitos.", Toast.LENGTH_SHORT);
								toastError.show();
								return false;
							}
						} else {
		...
		case ALERTA_IMPLEMENTAR:
			...
			builder2.setMessage(getString(R.string.registroPDVImplementarAlert)).setCancelable(true).setNegativeButton(R.string.textNo, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int id) {
					...
					dialog.dismiss();
					finish();
					Intent intrep = new Intent(RegistroPDVActivity.this, PuntoVentaSeleccion.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		outState.putString("dni", ((EditText) findViewById(R.id.etDNI)).getText().toString());
		outState.putString("ruc", ((EditText) findViewById(R.id.etRUC)).getText().toString());
		outState.putString("direccion", ((EditText) findViewById(R.id.etDireccion)).getText().toString());
		outState.putString("referencia", ((EditText) findViewById(R.id.etReferencia)).getText().toString());
		outState.putString("telefono", ((EditText) findViewById(R.id.etTelefono)).getText().toString());
		outState.putInt("departamento", ((Spinner) findViewById(R.id.spDepartamento)).getSelectedItemPosition());
		outState.putInt("provincia", ((Spinner) findViewById(R.id.spProvincia)).getSelectedItemPosition());
		outState.putInt("distrito", ((Spinner) findViewById(R.id.spDistrito)).getSelectedItemPosition());
		outState.putInt("tipo", ((Spinner) findViewById(R.id.spTipoPoblacion)).getSelectedItemPosition());
	...
	private void fillTipoPoblacionSpinner() {
		...
		spTipoDocumento.setAdapter(adap);

		spTipoDocumento.setOnItemSelectedListener(new OnItemSelectedListener() {
			public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id) {
				// your code here
				if (position == 0) {
					((EditText) findViewById(R.id.etDNI)).setVisibility(View.VISIBLE);
					((EditText) findViewById(R.id.etRUC)).setVisibility(View.GONE);
					((TextView) findViewById(R.id.textViewRUC)).setVisibility(View.GONE);
					((TextView) findViewById(R.id.textViewDNI)).setVisibility(View.VISIBLE);
				} else if (position == 1) {
					((EditText) findViewById(R.id.etDNI)).setVisibility(View.GONE);
					((EditText) findViewById(R.id.etRUC)).setVisibility(View.VISIBLE);
					((TextView) findViewById(R.id.textViewRUC)).setVisibility(View.VISIBLE);
					((TextView) findViewById(R.id.textViewDNI)).setVisibility(View.GONE);
				}
			}
		...
		public void handleMessage(Message msg) {
			...
			case 3:
				if (DatosManager.getInstancia().getPuntoVentaSeleccionado() != null) {
					DatosManager.getInstancia().setCodigosITT(null);
					finish();
					SQLiteDatabaseAdapter aSQLiteDatabaseAdapter = SQLiteDatabaseAdapter.getInstance(RegistroPDVActivity.this);
					db = aSQLiteDatabaseAdapter.getReadableDatabase();
					if (DatosManager.getInstancia().getUsuario() == null) {
						DatosManager instanciaDM = (DatosManager) getLastNonConfigurationInstance();
						if (instanciaDM == null) {
	...
	public void enviarNuevoPtoVenta() {
		...
		datosManager.enviarNuevaBodega_Mov(a, this, RegistroPDVActivity.this);
	}
...
//	}
	...
	public void mostrarMensaje(String msg) {
		indicadorProgreso = ProgressDialog.show(RegistroPDVActivity.this, "", msg, true);
	}
	...
	public void onBackPressed() {
		...
		super.onBackPressed();
		finish();
		Intent intrep = new Intent(RegistroPDVActivity.this, MenuBodegasActivity.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		startActivity(intrep);
	}
}
